...
<form class="uk-form uk-panel uk-panel-box" id="finder-search" action="<?php echo JRoute::_($this->query->toURI()); ?>" method="get">
	...
	<fieldset>
		...
		<div class="uk-form-row">
			<?php if ($this->escape($this->query->input) != '' || $this->params->get('allow_empty_search')):?>
				<button class="uk-button uk-button-primary" name="Search" type="submit" class="button"><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
			<?php else: ?>
				<button class="uk-button uk-button-primary" name="Search" type="" class="button"><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
			<?php endif; ?>
		</div>
