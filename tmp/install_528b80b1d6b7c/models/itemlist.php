...
class K2ModelItemlist extends JModel {
    ...
    function getData($ordering = NULL) {
		...
		if (JRequest::getInt('catid') != '') {
			$query .= " AND i.catid = ".JRequest::getInt('catid');
		}
        ...
        switch ($task) {
			...
			case 'filter':
				$badchars = array('#', '>', '<', '\\');
				$search = JRequest::getVar('searchword', null);
				// $search = trim(str_replace($badchars, '', JRequest::getVar('searchword', null)));
				if (! empty($search)) {
					$sql = K2ModelItemlist::prepareFilterArray($search, $badchars);
					if (! empty($sql)) {
						$query .= $sql;
					} else {
						$rows = array();
		...
		// ADDED K2FSM from here ->>
		// ADVANCED
		//$search = JRequest::getString('searchword', null);
		if ( JRequest::getInt('field_id') != "" && !empty($search) && count($rows) != 0 ) {
			require_once (JPATH_SITE.DS.'modules'.DS.'mod_k2_filter'.DS.'helper.php');
			
			$field_id = JRequest::getInt('field_id');
			$extra_field_type = modK2FilterHelper::pull($field_id, 'type');
			$extra_field_options = modK2FilterHelper::extractExtraFields(modK2FilterHelper::pull($field_id, 'value'));
			$results = array();
			
			foreach ($rows as $which=>$row) {
				$extra_fields_content2 = modK2FilterHelper::extractExtraFields(modK2FilterHelper::pullItem($row->id));
				$extra_fields_content = $extra_fields_content2[$field_id];

				if (is_array($search)) {
					foreach ($search as $searchitem) {
						if ( strpos(strtolower($extra_field_options[$extra_fields_content-1]), strtolower($searchitem)) !== false ) {
							$results[] = $rows[$which];
						}
					}
				} else {
					if ( strpos(strtolower($extra_field_options[$extra_fields_content-1]), strtolower($search)) !== false ) {
						$results[] = $rows[$which];
					}
				}	
				
			}
			// replace $rows with new results
			if (! empty($results)) {
				unset($rows);
				$rows = $results;
			}
	...
	function prepareFilterArray($searches2, $badchars) {
		jimport('joomla.filesystem.file');
		$db = &JFactory::getDBO();
		$language = &JFactory::getLanguage();
		$defaultLang = $language->getDefault();
		...
		// convert $searches to array if needed
		/* 
		(is_array($searches2) == false ) ?
			$searches[] = $searches2 :
			$searches = $searches2 ;
		*/
		( is_array($searches2) ) ?
			$search = implode(' ', $searches2) :
			$search = $searches2 ;
		
		// foreach ($searches as $search) {
			// $search = trim(str_replace($badchars, '', $search)); // Removed
			$length = JString::strlen($search);
			...
			if (JFolder::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish') && $currentLang != $defaultLang && JRequest::getVar('all_langs') != 'yes') {
				...
				if ($type=='exact') {
					...
					if (JString::strlen($word) > 3 && !in_array($word, $search_ignore)) {
						...
						$result = $db->loadResultArray();
						$result = @array_unique($result);
						JArrayHelper::toInteger($result); // added from SVN
						if (count($result)) {
							$conditions[] = "i.id IN(".implode(',', $result).")";
						}
					...
					foreach ($search as $searchword) {
						...
						if (JString::strlen($searchword) > 3 && !in_array($searchword, $search_ignore)) {
							...
							if (JFolder::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish') && $currentLang != $defaultLang) {
								...
								if (isset($allIDs) && count($allIDs)) {
									JArrayHelper::toInteger($allIDs); // added from SVN
									$conditions[] = "i.id IN(".implode(',', $allIDs).")";
								}
							}
